...
Assumptions:
1. Owner key is never compromised (same assumption as IdentityManager)
...
contract SimpleIdentity {
  ...
  event RecoveryChanged(bytes32 identifier, address recovery);
  ...
  mapping (bytes32 => Pending) pendingOwn;

  modifier onlyOwner(bytes32 identifier) {
    if (ownerOf[identifier] == msg.sender)  {
      if (pendingOwn[identifier].t != 0 && pendingOwn[identifier].t < now) {
        ownerOf[identifier] = pendingOwn[identifier].a;
        OwnerChanged(identifier, ownerOf[identifier]);
        delete pendingOwn[identifier];
      } else {
        _;
      }
    }
  ...
  */
  ...
  function changeOwnerAsRecovery(bytes32 identifier, address owner) {
    pendingOwn[identifier].a = owner;
    pendingOwn[identifier].t = now + WAIT_TIME; //careful for overflow
  }
}
